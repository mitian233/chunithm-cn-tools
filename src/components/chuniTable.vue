        ...
        <tr>
          <td :colspan="columns.length">
            <div style="padding: 15px; display: flex; flex-direction: row">
              <div>
                <img :src="getImageUrl(item._id)" loading="lazy" decoding="async" />
              </div>
              <div style="padding-left: 20px">
                ...
                <div style="padding-top: 20px">
                  <v-btn
                    ...
                    v-on:click="$router.push('/song/id/' + item._id)"
                  >
                    查看详细
                  </v-btn>
                </div>
...
export default {
  ...
  methods: {
    ...
    getDXScore(item) {
      let star = 5 - [97, 95, 93, 90, 85, 0].findIndex((v) => item.dxScore_perc >= v);
      let color = '';
    ...
    get_idx(ach) {
      return [50, 60, 70, 75, 80, 90, 94, 97, 98, 99, 99.5, 100, 100.5, 200].findIndex(
        (i) => ach < i
      );
    },
    get_l(idx) {
      return [0, 5, 6, 7, 7.5, 8.5, 9.5, 10.5, 12.5, 12.7, 13, 13.2, 13.5, 14][idx];
    },
    get_min_ach(idx) {
      return [0, 50, 60, 70, 75, 80, 90, 94, 97, 98, 99, 99.5, 100, 100.5, 101][idx];
    },
    get_ra(ds, ach) {
      return Math.floor((ds * this.get_l(this.get_idx(ach)) * Math.min(100.5, ach)) / 100);
    },
    getMoreRa(item) {
      let ds = item.ds;
    ...
    chartFilter(value, search, item) {
      ...
      return (
        search != null &&
        item.search_index.some((info) => info.indexOf(search.toLocaleLowerCase()) !== -1)
      );
    },
    gotoDsCalc(ds) {
      this.$router.push({
