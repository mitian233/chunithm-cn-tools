...
export const useMusicDataStore = defineStore('musicDataStore', () => {
  ...
  const chuni_records = shallowRef([]);
    ...
    for (const m of chuni_data.value) {
      for (let i = 0; i < m.ds.length; i++) {
        ...
        records.push({
          ...
          level_index: i,
          level_label:
            ['Basic', 'Advanced', 'Expert', 'Master', 'Ultima', "World's End"][i] +
            ' ' +
            m.level[i],
          from: chuni_data_dict.value[m.id].basic_info.from,
