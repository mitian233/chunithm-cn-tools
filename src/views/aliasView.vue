...
<template>
  <v-card>
    ...
    <v-card-subtitle>
      数据来源
      <a href="https://docs.qq.com/sheet/DY3FoeWRLdERLRUxB">
        https://docs.qq.com/sheet/DY3FoeWRLdERLRUxB
      </a>
    </v-card-subtitle>
    <v-container>
      <v-text-field
      ...
      <v-data-table
        :headers="headers"
        :items="store.alias_list"
        :search="searchKey"
        :loading="isLoading"
      >
        <template #item.id="{ item }">
          <router-link :to="`/song/id/${item.id}`">
            <p class="hover:underline">{{ item.id }}</p>
          </router-link>
        </template>
        <template #item.song_name="{ item }">
          <router-link :to="`/song/id/${item.id}`">
            <p class="hover:underline">{{ item.song_name }}</p>
          </router-link>
        </template>
